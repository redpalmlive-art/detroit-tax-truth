      ...
      <header className="border-b border-[#1A2E22] bg-[#0A1710] sticky top-0 z-50">
        <div className="max-w-[1200px] mx-auto px-6 py-3.5 flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-8 text-[11px] tracking-[0.18em] text-[#5F8570]">
            <a href="/" className="text-[#E8EDE9]">REMEDY ENGINE</a><a href="/estimate">TAX ESTIMATOR</a><a href="/accountability">ACCOUNTABILITY</a>
          </nav>
        </div>
      </header>

      <main className="max-w-[1200px] mx-auto px-6 pt-6 pb-10 space-y-5">
        {/* HERO - NOW SAME SIZE AS ACCOUNTABILITY - SHORTENED TO SHOW SEARCH */}
        <section className="border border-[#E7C369]/15 bg-[#122219] rounded-[28px] px-8 py-8 md:px-10 md:py-9">
          <div className="text-[10px] tracking-[0.22em] text-[#E7C369]/90 font-medium">CITYWIDE INCLUSION LEDGER</div>
        ...
        {/* SEARCH - NOW VISIBLE WITHOUT SCROLL - SAME SIZE AS ACCOUNTABILITY */}
        <section className="bg-[#F5F1E8] rounded-[20px] p-2.5 flex gap-2.5">
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter' && setSearched(true)} className="flex-1 bg-transparent text-black px-6 py-3 rounded-full outline-none text-[14px] placeholder:text-black/40" placeholder="1470 Atkinson St, Detroit, MI 48206" />
        ...
        {searched && (
          <section className="border border-[#1E3A2A] bg-[#0F1F16] rounded-[20px] overflow-hidden">
            <div className="px-7 py-4 flex justify-between text-[11px] border-b border-[#1A2E22]"><span className="tracking-widest text-[#E7C369]">OVERCHARGE 2010-2016 • {input}</span><span className="font-bold">${overByYear.reduce((s,r)=>s+r.over,0).toLocaleString()} over</span></div>
